...
const Calendar = (props) => {
  ...
  const createSpan = (text, current = false) => (
    <span
      ...
      onClick={(e) => {
        let clone = moment(selectedDay, "DD-MM-YYYY").set({
  ...
  const createDays = () => {
    ...
    let rows = Array(myDate.startOf("month").day())
      .fill(0)
      .map((_, day) => (
        <li name={day + 1} style={liStyle} key={day + 1}>
          {createSpan("")}
        </li>
      ));
    ...
    for (let i = 1; i <= daysInMonth; i++) {
      rows.push(
        <li name={i} style={liStyle} key={i * 10}>
          {createSpan(i, i === clone.date())}
        </li>
      );
    }
    return rows;
